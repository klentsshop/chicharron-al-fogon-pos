...
export default function MenuPanel() {
    ...
    // ESTADOS PARA MÓVIL (RAPPI LOGIC)
    ...
    const guardarOrden = async () => {
        ...
        let mesero = nombreMesero || prompt("👤 Nombre del Mesero:", "Diana");
        if (!mesa || !mesero) return;
        try {
            await apiGuardar({ 
                mesa, mesero, ordenId: ordenActivaId, 
    ...
    const cargarOrden = async (id) => {
        try {
            ...
            setCartFromOrden(o.platosOrdenados.map(p => ({ 
                nombrePlato: p.nombrePlato, precioUnitario: p.precioUnitario, cantidad: p.cantidad 
            })));
    ...
    return (
        <div className={styles.mainWrapper}>
            <div className={styles.posLayout}>
                {/* 1. CSS IMPRESIÓN 80MM */}
                <style dangerouslySetInnerHTML={{ __html: `
                    #ticket-print { position: fixed; top: -5000px; left: -5000px; width: 80mm; }
                    @media print {
                ...
                <TicketPanel 
                    ...
                    ordenMesa={ordenMesa} nombreMesero={nombreMesero} esModoCajero={esModoCajero}
                    ordenActivaId={ordenActivaId} numOrdenesActivas={ordenesActivas.length} cleanPrice={cleanPrice} styles={styles}
                    cancelarOrden={cancelarOrden} imprimirTicket={imprimirTicketMesa}
                />
                ...
                <ProductGrid 
                    ...
                    mostrarCategoriasMobile={mostrarCategoriasMobile} 
                    setMostrarCategoriasMobile={setMostrarCategoriasMobile}
                    agregarAlCarrito={agregarAlCarrito} 
                    styles={styles}
                    mostrarCarritoMobile={mostrarCarritoMobile}
                    setMostrarCarritoMobile={setMostrarCarritoMobile}
                />
            ...
            {/* --- NUEVA BARRA DE MESAS ACTIVAS (FOOTER) --- */}
